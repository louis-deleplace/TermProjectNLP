...
def putIntoTable(title, artist, year, lyrics, genre, popularity, bpm, energy, danceability, song_length):
    try:
        ...
        record = (title, artist, year, lyrics, genre, popularity, bpm, energy, danceability, song_length)
...
def putDataInTable():
    with open('Spotify-2000.csv', 'r') as csvfile:
        ...
        for row in reader:
            putIntoTable(row['Title'], row['Artist'], int(row['Year']), "", row['Top Genre'], row['Popularity'], row['Beats Per Minute (BPM)'], row['Energy'], row['Danceability'], row['Length (Duration)'])
...
def getAllLyrics():
    ...
    for row in records:
        try:
            url = "http://api.chartlyrics.com/apiv1.asmx/SearchLyricDirect?artist=" + row[1].lower() + "&song=" + row[0].lower()
